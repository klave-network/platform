...
type Versions = Pick<PackageJson, 'dependencies' | 'devDependencies'>;
...
export function info() {
    ...
    infoCmd.action(() => {
        ...
        const filterByScope = (deps: PackageJson['dependencies']) => {
            if (!deps) return [];
            return Object.entries(deps).filter(([name]) => name.startsWith('@klave/'));
        };
