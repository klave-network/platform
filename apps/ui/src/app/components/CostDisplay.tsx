...
const CostDisplay: FC<CostDisplayProps> = ({ basis, compact, size, justify, className }) => {
    ...
    if (compact)
        return <span className={className}><span className={'text-klave-light-blue'}>{isNonNilTinyAmount ? '~' : ''}£{roundedCurrencyValue}</span> <span className="text-slate-300 text-xs">({(type === 'wasm' || type === 'native') ? `${consumptionValue.toString()} instructions` : prettyBytes(consumptionValue / 10_000, { maximumFractionDigits: 2 })})</span></span>;
    ...
    return <span className={`flex flex-col flex-shrink justify-start items-${justify ?? 'start'} ${className ?? ''}`} >
        ...
        <span className="text-slate-300 text-xs">{(type === 'wasm' || type === 'native') ? `${consumptionValue.toString()} instructions` : prettyBytes(consumptionValue / 10_000, { maximumFractionDigits: 2 })}</span>
    </span>;
};
