...
const CostDisplay: FC<CostDisplayProps> = ({ consumption, type, scope, compact, size, justify, className }) => {
    ...
    if (compact)
        return <span className={className}><span className={'text-klave-light-blue'}>{isNonNilTinyAmount ? '~' : ''}£{roundedCurrencyValue}</span> <span className="text-slate-300 text-xs">({consumptionNumber.toString()} {(type === 'wasm' || type === 'native') ? 'instructions' : prettyBytes(consumptionNumber, { maximumFractionDigits: 2 })})</span></span>;
    ...
    return <span className={`flex flex-col flex-shrink justify-start items-${justify ?? 'start'} ${className ?? ''}`} >
        ...
        <span className="text-slate-300 text-xs">{(type === 'wasm' || type === 'native') ? `${consumptionNumber.toString()} instructions` : prettyBytes(consumptionNumber, { maximumFractionDigits: 2 })}</span>
    </span>;
};
