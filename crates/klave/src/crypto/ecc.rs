...
use super::sdk_wrapper::{Key, CryptoImpl, VerifySignResult};
use super::subtle_idl_v1::{SecpR1Metadata, EcdsaSignatureMetadata, ShaMetadata};
use super::subtle_idl_v1_enums::{KeyAlgorithm, ShaAlgorithm, ShaDigestBitsize, SecpR1KeyBitsize, SigningAlgorithm};
...
impl Default for KeyECC {
    fn default() -> Self {
        KeyECC { key: Key::new(&String::from("")), named_curve: String::from("P-256") }
    }
}

impl Display for KeyECC {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "KeyECC: name: {}, named_curve: {}", self.key.name(), self.named_curve)
    }
}
...
impl KeyECC 
{
    pub fn new(name: &str, named_curve: &str) -> KeyECC {
        KeyECC { key: Key::new(name), named_curve: named_curve.to_string() }
    }
