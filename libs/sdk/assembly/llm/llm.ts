...
import * as LlmIdl from './llm_idl_v1';
...
declare function wasm_inference_compute(context_name: ArrayBuffer, input_tensor: ArrayBuffer, output_tensor: ArrayBuffer, output_tensor_size: i32): i32;
...
function LoadStatusToString(status: LlmIdl.LoadStatus): string
{
    switch (status) {
        case LlmIdl.LoadStatus.LOADED_IN_RAM:
            return "LOADED_IN_RAM";
        case LlmIdl.LoadStatus.UNLOADED_FROM_RAM:
            return "UNLOADED_FROM_RAM";
        case LlmIdl.LoadStatus.FAILED:
            return "FAILED";
        default:
            return "UNKNOWN";
    }
...
function graphLoad(model_name: string, encoding: i32, target: i32): Result<string, Error>
{
    let error = new ArrayBuffer(64);
    let result = wasm_graph_load(String.UTF8.encode(model_name, true), encoding, target, error, error.byteLength);
    if (abs(result) > error.byteLength) {
        // buffer not big enough, retry with a properly sized one
        error = new ArrayBuffer(abs(result));
        result = wasm_graph_load(String.UTF8.encode(model_name, true), encoding, target, error, error.byteLength);
    }
    if (result < 0)
        return { data: LoadStatusToString(LlmIdl.LoadStatus.FAILED), err: new Error(String.UTF8.decode(error.slice(0, -result))) };
    return { data: LoadStatusToString(LlmIdl.LoadStatus.LOADED_IN_RAM), err: null };
}
...
export function graphUnloadByName(model_name: string): Result<string, Error>
{
    ...
    if (result < 0)
        return { data: LoadStatusToString(LlmIdl.LoadStatus.FAILED), err: new Error(String.UTF8.decode(error.slice(0, -result))) };
    return { data: LoadStatusToString(LlmIdl.LoadStatus.UNLOADED_FROM_RAM), err: null };
}
...
export function graphInitExecutionContext(metadata: string): Result<string, Error>
{
    ...
    return { data: "Context initialized with " + metadata, err: null };
}
...
export function inferenceCompute(context_name: string, input_tensor: string): Result<string, Error>
{
    let output_tensor = new ArrayBuffer(1024);
    let result = wasm_inference_compute(String.UTF8.encode(context_name, true), String.UTF8.encode(input_tensor, true), output_tensor, output_tensor.byteLength);
    if (abs(result) > output_tensor.byteLength) {
        // buffer not big enough, retry with a properly sized one
        output_tensor = new ArrayBuffer(abs(result));
        result = wasm_inference_compute(String.UTF8.encode(context_name, true), String.UTF8.encode(input_tensor, true), output_tensor, output_tensor.byteLength);
    }
    if (result < 0)
