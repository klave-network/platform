...
class CryptoAES {
    ...
    static generateKey(keyName: string = "", extractable = false): KeyAES {
        ...
        generate_key(nameBuf, 1 /* AES-GCM */, extractable, usages)
        return key;
    }
    ...
    static importKey(keyName: string = "", keyData: string, format = 'raw', extractable = false): KeyAES | null {
        ...
        const result = import_key_raw(nameBuf, iFormat, rawData.buffer, rawData.byteLength, 1 /* AES-GCM */, extractable ? 1 : 0, usages, usages.length);
        if (result < 0)
            return null;
...
class CryptoECDSA {
    ...
    static generateKey(keyName: string = "", extractable = false): KeyECC {
        ...
        generate_key(nameBuf, 0 /* ECDSA */, extractable, usages)
        return key;
    }
    ...
    static importKey(keyName: string = "", keyPair: {
        private?: string,
        public?: string
    }, format = 'raw', extractable = false): KeyECC | null {

        if (keyPair.private && keyPair.public)
            return null;

        const keyBuff = keyPair.private ?? keyPair.public;
        ...
        const usages = new Uint8Array(2);
        usages[0] = keyPair.private ? 2 : 3

        const result = import_key_raw(nameBuf, iFormat, rawData.buffer, rawData.byteLength, 0 /* ECDSA */, extractable ? 1 : 0, usages, usages.length);
        if (result < 0)
            return null;
